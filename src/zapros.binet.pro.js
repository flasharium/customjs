...
if (window.location.href.search(/key=/) != -1) {
  ...
  if (obj['strong_equals'].length) {
  	text += '3. Фраза "' +obj['strong_equals'][0].replace(/\(.*\)/, '')+ '" обязательно должна быть вписана в первый абзац в точном виде \n';
  } else if (obj['not_equals'].length) {
  	text += '3. Фраза "' +obj['not_equals'][0].replace(/\(.*\)/, '')+ '" обязательно должна быть вписано в первый абзац с изменением окончаний \n';
  }
